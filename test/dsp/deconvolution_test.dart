...
void main() {
  test('deconvolution recovers the relative frequency response of a known FIR "speaker"', () {
    ...
    final sweep = SignalGenerator.sineSweep(f0: f0, f1: f1, durationS: duration, sampleRate: sampleRate);
    ...
    final expected = FftUtils.magnitudeSpectrum(fir, sampleRate, hannWindow: false, fftSize: 65536);
    ...
    for (final testFreq in [300.0, 1000.0, 2000.0, 4000.0, 8000.0]) {
      final expectedRel = _dbAtNearestFreq(expectedResponse, testFreq) - expectedRef;
      final recoveredRel = _dbAtNearestFreq(recovered, testFreq) - recoveredRef;
      expect(
        recoveredRel,
        closeTo(expectedRel, 3.0),
        reason: 'at ${testFreq}Hz: expected ${expectedRel}dB relative, got ${recoveredRel}dB relative',
      );
    }
  });
}
