...
/// sweep math.
double _zeroCrossingFreq(List<double> samples, int start, int length, int sampleRate) {
  int crossings = 0;
...
double _expectedAvgFreqLog(double f0, double f1, double durationS, double t1, double t2) {
...
double _expectedAvgFreqLinear(double f0, double f1, double durationS, double t1, double t2) {
  double phase(double t) => 2 * math.pi * (f0 * t + (f1 - f0) * t * t / (2 * durationS));
  return (phase(t2) - phase(t1)) / (2 * math.pi * (t2 - t1));
}
...
void main() {
  ...
  group('SignalGenerator.sineSweep', () {
    test('log sweep instantaneous frequency matches the analytic sweep law', () {
      const f0 = 100.0, f1 = 8000.0, duration = 2.0;
      final s = SignalGenerator.sineSweep(f0: f0, f1: f1, durationS: duration, sampleRate: sampleRate);
      ...
      const startOffset = 500, startWindowLen = 4410; // ~10 cycles at f0=100Hz
      final measuredStart = _zeroCrossingFreq(s, startOffset, startWindowLen, sampleRate);
      ...
      final measuredEnd = _zeroCrossingFreq(s, endOffset, windowLen, sampleRate);
    ...
    test('linear sweep instantaneous frequency matches the analytic sweep law', () {
      ...
      final measuredStart = _zeroCrossingFreq(s, startOffset, windowLen, sampleRate);
  ...
  group('SignalGenerator noise', () {
    test('pink noise has ~ -3dB/octave spectral slope', () {
      ...
      final band = spectrum.points.where((p) => p.freqHz >= 100 && p.freqHz <= 8000).toList();
  ...
  group('SignalGenerator.applyLevelDbfs', () {
    test('scales peak amplitude to the configured dBFS level', () {
      final tone = SignalGenerator.pureTone(freq: 1000, durationS: 0.1, sampleRate: sampleRate);
