  ...
  group('CrossCorrelation.findOffset', () {
    test('recovers exact offset of a chirp embedded in silence', () {
      ...
      final recorded = <double>[
        ...List.filled(offset, 0.0),
        ...chirp,
        ...List.filled(5000, 0.0),
      ];
    ...
    test('is robust to additive noise and a gain change', () {
      ...
      final recorded = List<double>.generate(
        offset,
        (_) => (rand.nextDouble() - 0.5) * 0.05,
      );
      recorded.addAll(chirp.map((s) => s * 0.6));
      recorded.addAll(
        List.generate(5000, (_) => (rand.nextDouble() - 0.5) * 0.05),
      );
    ...
    test(
      'locates the main test signal after the calibration chirp in a combined buffer',
      () {
        final chirp = SignalGenerator.calibrationChirp(sampleRate: sampleRate);
        ...
        final mainSignalStart =
            result.offsetSamples + chirp.length + gap.length;
