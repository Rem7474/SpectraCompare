...
class FakeRecorder implements Recorder {
  ...
  FakeRecorder(this.link, {this.injectedOffsetSamples = 800, this.sampleRate = 44100});
  ...
  Future<WavData> stop() async {
    ...
    if (played == null) {
      return WavData(sampleRate: sampleRate, channels: 1, samples: Float64List(0));
    }
    final decoded = WavDecoder.decode(played);
    final pre = Float64List(injectedOffsetSamples);
    final post = Float64List(400);
    final combined = Float64List(pre.length + decoded.samples.length + post.length);
...
void main() {
  ...
  test('run() locates the chirp and extracts the main signal segment despite an injected acoustic delay', () async {
    final link = FakeAudioLink();
    const injectedOffset = 800;
    final session = MeasurementSession(
      recorder: FakeRecorder(link, injectedOffsetSamples: injectedOffset, sampleRate: sampleRate),
    ...
    final prerollSamples = (fastTimings.preroll.inMicroseconds * sampleRate / 1e6).round();
    expect(result.offsetSamples, injectedOffset + prerollSamples);
    expect(result.confidence, greaterThan(0.9));

    final expectedMainSignal = SignalGenerator.generate(signalConfig, sampleRate);
    expect(result.mainSignalSegment.length, expectedMainSignal.length);
    for (int i = 0; i < expectedMainSignal.length; i += 500) {
      expect(result.mainSignalSegment[i], closeTo(expectedMainSignal[i], 1e-3));
    }
  });

  test('buildCombinedWav lays out [preroll][chirp][gap][main][tail] at the expected sample positions', () {
    final session = MeasurementSession(
    ...
    const signalConfig = SignalConfig(type: SignalType.pureTone, frequencyHz: 1000, durationS: 0.2);
    ...
    final prerollSamples = (fastTimings.preroll.inMicroseconds * sampleRate / 1e6).round();
    final gapSamples = (fastTimings.gap.inMicroseconds * sampleRate / 1e6).round();
    final tailSamples = (fastTimings.tail.inMicroseconds * sampleRate / 1e6).round();

    final expectedLength = prerollSamples + chirp.length + gapSamples + mainSignal.length + tailSamples;
