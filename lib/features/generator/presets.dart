...
class SignalPreset {
  ...
  const SignalPreset({required this.name, required this.description, required this.config});
}
...
class SignalPresets {
  ...
  static const sweepStandard = SignalPreset(
    name: 'Sweep standard',
    description: 'Balayage logarithmique 20Hz–20kHz, 10s — mesure de réponse en fréquence',
    config: SignalConfig(
  ...
  static const calibrationPinkNoise = SignalPreset(
    ...
    config: SignalConfig(type: SignalType.pinkNoise, durationS: 10, levelDbfs: -20),
  );

  static const testRattleBurst = SignalPreset(
    name: 'Test rattle/burst',
    description: 'Impulsion courte basse fréquence — détection de vibrations/rattle',
    config: SignalConfig(type: SignalType.burst, frequencyHz: 80, durationS: 0.3, levelDbfs: -20),
  );
